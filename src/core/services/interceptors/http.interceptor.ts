    ...
    try {
      ...
      if (!expectedError) {
        try {
          ...
          showToast([error.response.data.message[0]], ToastTypes.error);
        } catch (er) {
          ...
          try {
            if (errorStatus === 403) {
              ...
              !IsIncludes(error.config.url, "/at") &&
              !IsIncludes(error.config.url, "ae") &&
              !IsIncludes(error.config.url, "ap")
            )
              showToast(
            ...
            if (
              !IsIncludes(error.config.url, "/") &&
              !IsIncludes(error.config.url, "") &&
              !IsIncludes(error.config.url, "e")
            )
              showToast(
                ["لطفا مواردی که دارای مشکل هستند را بررسی کنید"],
                ToastTypes.error
